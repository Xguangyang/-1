...
namespace TMS.Repository.Personnel
{
    ...
    /// 离职办理
    /// </summary>
    public class PositiveToDealRepository : IPositiveToDealRepository
    {
        ...
        //小型工厂模式
        public PositiveToDealRepository(IDapperFactory dapperFactory)
        {
        ...
        public async Task<List<PositiveToDealViewModel>> GetPositiveToDealViewModels(string EmpName, string EmpDeparName, string PosterName, DateTime? EntryTime, DateTime? ProposerTime, int ExamineStatus)
        {
            ...
            List<PositiveToDealViewModel> data = await _SqlDB.QueryAsync<PositiveToDealViewModel>(sql);
            //查询
            ...
            {
                data = data.Where(m => m.EmployeeProposerTime.Equals(ProposerTime)).ToList();
            }
            if (ExamineStatus != 0)
            {
